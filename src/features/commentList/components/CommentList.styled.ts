...
export const Wrapper = styled.div<{ $isOpen: boolean }>(({ $isOpen }) => [
  ...
  `,
  tw`hidden flex-col bg-white border-l border-gray-200 fixed top-0 right-0 z-30 max-w-sm w-full h-dvh p-4 shadow-md gap-4 overflow-y-scroll transition duration-300`,
  $isOpen && tw`flex`,
]);
